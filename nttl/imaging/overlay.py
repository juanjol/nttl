...
class OverlayConfig(BaseModel):
    enabled: bool = True
    margin: int = Field(default=8, ge=0, le=200)
...
def render_overlay(
    ...
    for item in config.items:
        ...
        if not text:
            continue
        font = _font(config, item.font_size)
        ...
        if item.position in (Position.TOP_LEFT, Position.BOTTOM_LEFT):
            ...
            x = int(width - config.margin - text_w)
        if item.position in (Position.TOP_LEFT, Position.TOP_RIGHT):
            y = int(config.margin + offset)
        else:
            y = int(height - config.margin - text_h - offset)
        cursors[item.position] = offset + text_h + config.line_spacing
        if item.background:
            pad = max(2, item.font_size // 5)
            draw.rectangle(
